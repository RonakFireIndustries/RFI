...
<div><div class="s-card"><div class="n">{{ $data['totalRecords'] }}</div><div class="l">Records</div></div></div>

<table>
<tr><th>Employee</th><th>Date</th><th>In</th><th>Out</th><th class="num">Hours</th><th>Status</th></tr>
@foreach($data['records'] as $a)
<tr>
  <td>{{ $a->employee->full_name ?? $a->employee->name ?? 'N/A' }}</td>
  <td>{{ $a->date->format('d-m-Y') }}</td>
  <td>{{ $a->check_in ? $a->check_in->format('H:i') : '-' }}</td>
  <td>{{ $a->check_out ? $a->check_out->format('H:i') : '-' }}</td>
  <td class="num">{{ number_format($a->working_hours ?? 0, 1) }}</td>
  <td>{{ $a->status ?? 'N/A' }}</td>
</tr>
@endforeach
</table>
