...
<div><div class="s-card"><div class="n">{{ $data['total'] }}</div><div class="l">Total Employees</div></div></div>

@if($data['byDepartment']->count())
<h3>By Department</h3>
<table>
<tr><th>Department</th><th class="num">Employees</th></tr>
@foreach($data['byDepartment'] as $dept => $cnt)
<tr><td>{{ $dept }}</td><td class="num">{{ $cnt }}</td></tr>
@endforeach
</table>
@endif

@if($data['byType']->count())
<h3>By Employment Type</h3>
<table>
<tr><th>Type</th><th class="num">Employees</th></tr>
@foreach($data['byType'] as $type => $cnt)
<tr><td>{{ $type }}</td><td class="num">{{ $cnt }}</td></tr>
@endforeach
</table>
@endif
