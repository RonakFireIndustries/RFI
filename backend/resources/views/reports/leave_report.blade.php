...
<div>
  <div class="s-card"><div class="n">{{ $data['totalLeaves'] }}</div><div class="l">Total Leaves</div></div>
  <div class="s-card"><div class="n">{{ $data['pendingCount'] }}</div><div class="l">Pending</div></div>
  <div class="s-card"><div class="n">{{ $data['approvedCount'] }}</div><div class="l">Approved</div></div>
</div>

<table>
<tr><th>Employee</th><th>Leave Type</th><th>From</th><th>To</th><th class="num">Days</th><th>Status</th></tr>
@foreach($data['leaves'] as $l)
<tr>
  <td>{{ $l->employee->full_name ?? $l->employee->name ?? 'N/A' }}</td>
  <td>{{ $l->leaveType->name ?? 'N/A' }}</td>
  <td>{{ $l->start_date->format('d-m-Y') }}</td>
  <td>{{ $l->end_date->format('d-m-Y') }}</td>
  <td class="num">{{ number_format($l->total_days, 1) }}</td>
  <td>{{ $l->status ?? 'N/A' }}</td>
</tr>
@endforeach
</table>
