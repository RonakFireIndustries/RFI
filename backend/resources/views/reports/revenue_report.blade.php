...
<div>
  <div class="s-card"><div class="n">&#8377;{{ number_format($data['totalRevenue'], 0) }}</div><div class="l">Total Revenue</div></div>
  <div class="s-card"><div class="n">{{ $data['totalInvoices'] }}</div><div class="l">Invoices</div></div>
  <div class="s-card"><div class="n">{{ $data['paidInvoices'] }}</div><div class="l">Paid</div></div>
  <div class="s-card"><div class="n">&#8377;{{ number_format($data['pendingAmount'], 0) }}</div><div class="l">Pending</div></div>
</div>

<table>
<tr><th>Invoice #</th><th>Customer</th><th>Date</th><th class="num">Amount</th><th>Status</th></tr>
@foreach($data['invoices'] as $inv)
<tr>
  <td>{{ $inv->invoice_number ?? $inv->id }}</td>
  <td>{{ $inv->customer->name ?? 'N/A' }}</td>
  <td>{{ $inv->created_at->format('d-m-Y') }}</td>
  <td class="num">&#8377;{{ number_format($inv->grand_total ?? 0, 0) }}</td>
  <td>{{ $inv->status ?? 'N/A' }}</td>
</tr>
@endforeach
</table>

<div class="footer">RFI Management Suite &mdash; Financial &mdash; Confidential</div>
</body>
</html>
