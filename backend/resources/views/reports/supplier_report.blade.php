...
<div><div class="s-card"><div class="n">{{ $data['totalSuppliers'] }}</div><div class="l">Total Suppliers</div></div></div>

<table>
<tr><th>Supplier</th><th>Contact</th><th>Email</th><th class="num">Purchase Orders</th></tr>
@foreach($data['suppliers'] as $s)
<tr>
  <td>{{ $s->name }}</td>
  <td>{{ $s->contact_person ?? $s->phone ?? 'N/A' }}</td>
  <td>{{ $s->email ?? 'N/A' }}</td>
...
<div class="footer">RFI Management Suite &mdash; Purchases &mdash; Confidential</div>
</body>
</html>
