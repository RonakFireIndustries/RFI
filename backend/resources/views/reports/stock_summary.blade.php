...
<div>
  <div class="s-card"><div class="n">{{ $data['totalProducts'] }}</div><div class="l">Products</div></div>
  <div class="s-card"><div class="n">{{ number_format($data['totalQty'], 0) }}</div><div class="l">Total Qty</div></div>
</div>

<table>
<tr><th>Product</th><th>Location</th><th class="num">Quantity</th><th class="num">Reserved</th></tr>
@foreach($data['stocks'] as $s)
<tr>
  <td>{{ $s->product->name ?? 'N/A' }}</td>
  <td>{{ $s->locationable->name ?? $s->location_type ?? 'N/A' }}</td>
  <td class="num">{{ number_format($s->quantity, 0) }}</td>
  <td class="num">{{ number_format($s->reserved_quantity ?? 0, 0) }}</td>
</tr>
@endforeach
</table>
