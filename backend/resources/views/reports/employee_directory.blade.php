...
<div>
  <div class="s-card"><div class="n">{{ $data['total'] }}</div><div class="l">Total Employees</div></div>
</div>

<table>
<tr><th>Employee</th><th>Department</th><th>Designation</th><th>Contact</th><th>Employment Type</th></tr>
@foreach($data['employees'] as $e)
<tr>
  <td>{{ $e->full_name ?? $e->name ?? 'N/A' }}</td>
  <td>{{ $e->department->name ?? 'N/A' }}</td>
  <td>{{ $e->designation->name ?? 'N/A' }}</td>
  <td>{{ $e->contact_number ?? 'N/A' }}</td>
  <td>{{ $e->employment_type ?? 'N/A' }}</td>
</tr>
@endforeach
</table>
