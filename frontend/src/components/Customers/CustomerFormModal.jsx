...
export default function CustomerFormModal({ isOpen, onClose, onSuccess, customer = null }) {
  ...
  useEffect(() => {
    if (customer) {
      setFormData({
        ...
        branch_id: customer.branch_id || user?.branch_id
      });
    } else {
      setFormData({
        name: '',
  ...
  const validate = () => {
    ...
    // Basic GST format validation (India GST format usually 15 chars alphanumeric)
    if (formData.gst_number && !/^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}$/i.test(formData.gst_number)) {
      // For testing, we might be lenient, but the prompt asked for "GST Format Validation"
      // Wait, let's keep it simple or show a warning. We will enforce standard 15 chars if present.
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="e.g. NexusTech Solutions Inc."
            />
